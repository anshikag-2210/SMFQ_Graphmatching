...
SC2LQ::SC2LQ(std::shared_ptr<BipartiteGraph> G,
    bool A_proposing)
...
bool SC2LQ::floyd_warshall(std::vector<std::vector<int>>& weight, int num_of_vertices,
...
// returns true if negative edge cycle is found
bool SC2LQ::bellman_ford(std::shared_ptr<BipartiteGraph> G,
...
bool SC2LQ::is_popular(std::shared_ptr<BipartiteGraph> G,
...
bool SC2LQ::level_proposing(std::shared_ptr<MatchingAlgorithm::MatchedPairListType>& M,
...
IdType SC2LQ::get_node_name(IdType id1, IdType id2, IdType id3) {
    return id1 + "_" + id2 + "_" + id3;
}

// To print brandl kavitha graph for SM2LQ instance
void SC2LQ::generate_brandl_kavitha_graph_sm2lq(std::shared_ptr<BipartiteGraph> G,
    std::shared_ptr<MatchingAlgorithm::MatchedPairListType>& M) {
...
void SC2LQ::generate_brandl_kavitha_graph_hr2lq(std::shared_ptr<BipartiteGraph> G,
    std::shared_ptr<MatchingAlgorithm::MatchedPairListType>& M) {
...
bool SC2LQ::compare(std::vector<int> A, std::vector<int> B) {
...
std::shared_ptr<BipartiteGraph> SC2LQ::reduce_partition_to_SM(BipartiteGraph::ContainerType A,
...
std::shared_ptr<BipartiteGraph> SC2LQ::reduce_partition_to_HR(BipartiteGraph::ContainerType A,
    BipartiteGraph::ContainerType B, bool is_R_phase) {
...
std::shared_ptr<BipartiteGraph> SC2LQ::get_reduced_graph() {
...
/*std::shared_ptr<MatchingAlgorithm::MatchedPairListType> SC2LQ::compute_matching() {
...
std::shared_ptr<MatchingAlgorithm::MatchedPairListType> SC2LQ::compute_matching() {
    ...
    //get sum of lower quota for both student and course.
    int t=0,s=0;
    //find sum of lower quotas of partiton A
    for (auto& A1 : A_old) {
        auto u = A1.second;
